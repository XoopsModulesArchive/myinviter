...
class nz11 extends OpenInviter_Base
	...
	public $debug_array=array('initial_get'=>'loginName',
							  'post_login'=>'eon-app-loading-wrapper',
	...
	public function login($user, $pass)
	{
		...
		$res=$this->get("http://nz11.com/");
		if ($this->checkResponse('initial_get',$res))
			$this->updateDebugBuffer('initial_get',"http://nz11.com/",'GET');
		else 
			{
			$this->updateDebugBuffer('initial_get',"http://nz11.com/",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;	
			}
			
		$form_action='http://nz11.mail.everyone.net/email/scripts/loginuser.pl';
		...
		foreach ($contacts_array as $contacts_string)
			if (strpos($contacts_string,'javascript:composeMe')!==false)
				...
				if (isset($contacts_explode[1]))
					$contacts[$contacts_explode[1]]=(isset($contacts_explode[0])?$contacts_explode[0]:false);
				}		
