...
class OpenInviter
{
    ...
    public function __construct()
    {
        ...
        $this->settings = array(
            ...
            'transport'=>'curl',
            'local_debug'=> false, //options : 'on_error' or 'allways', be very careful using this
            'remote_debug'=>'false');
    }
    ...
    {
        if (file_exists(dirname(dirname(__FILE__))."/plugins/{$plugin_name}.php")) {
            $ok = true;
            if (!class_exists($plugin_name)) {
                include_once(dirname(dirname(__FILE__))."/plugins/{$plugin_name}.php");
            }
    ...
    private function checkLoginCredentials($user)
    {
        ...
        if ($this->plugin->allowed_domains AND $is_email) {
            ...
            foreach ($this->plugin->allowed_domains as $domain) {
                if (strpos($user_domain,$domain) !== false) {
                    $temp=true;
                }
            }
            if (!$temp) {
